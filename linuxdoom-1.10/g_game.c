...
{ 
    ...
    #endif 
        // Changed from 3 separate if statements
        // 3 cases for early return
        // chat ate the event 
        // status window ate it
        // automap ate it 
        if (HU_Responder (ev) || ST_Responder (ev) || AM_Responder (ev)) {
            return true;
        }
    } 
...
{ 
    ...
    if (strcmp (save_p, vcheck)) return; // bad version 
    save_p += VERSIONSIZE; 
